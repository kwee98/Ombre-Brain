...
- 如果带了 source_bucket，只更新源记忆的 model_valence（不标 digested，
  源靠 resolved 自己决定何时沉底——年轮），并自动建 feel→source 边（骨头配套）
...
async def store_feel(
    ...
    if source_bucket and source_bucket.strip():
        src = source_bucket.strip()
        try:
            # 年轮：feel 写入不加速源记忆沉底（不标 digested），
            # 源只跟 resolved 状态挂钩；这里只记录「我视角的 valence」。
            update_kwargs: dict[str, float] = {}
            if 0 <= valence <= 1:
                update_kwargs["model_valence"] = feel_valence
            if update_kwargs:
                await rt.bucket_mgr.update(src, **update_kwargs)
        except Exception as e:
            rt.logger.warning(f"Failed to update source model_valence: {e}")
        try:
            # 骨头：feel → source 自动建边（edge_type="triggered"）
            rt.bucket_mgr.add_edge(bucket_id, src, edge_type="triggered", weight=1.0)
        except Exception as e:
            rt.logger.warning(f"Failed to add feel→source edge: {e}")
