...
class BucketManager:
    ...
    def _init_edge_table(self) -> None:
        try:
            import os as _os
            _os.makedirs(_os.path.dirname(self._edge_db_path), exist_ok=True)
            import sqlite3 as _s3
            conn = _s3.connect(self._edge_db_path)
            conn.execute("""
            ...
            import logging; logging.getLogger(__name__).warning(f"Edge table init failed: {e}")
    ...
    def add_edge(self, source_id: str, target_id: str, edge_type: str = "related", weight: float = 1.0) -> None:
        try:
            ...
            ts = now_iso()
            conn = _s3.connect(self._edge_db_path)
            conn.execute("INSERT OR REPLACE INTO edges (source_id, target_id, edge_type, weight, created_at) VALUES (?,?,?,?,?)", (source_id, target_id, edge_type, weight, ts))
            conn.execute("INSERT OR REPLACE INTO edges (source_id, target_id, edge_type, weight, created_at) VALUES (?,?,?,?,?)", (target_id, source_id, edge_type, weight, ts))
            conn.commit()
            conn.close()
        except Exception as e:
            import logging; logging.getLogger(__name__).warning(f"add_edge failed: {e}")

    def list_edges(self, bucket_id: str) -> list:
        try:
            import sqlite3 as _s3
            conn = _s3.connect(self._edge_db_path)
            rows = conn.execute("SELECT target_id, edge_type, weight, created_at FROM edges WHERE source_id=? ORDER BY weight DESC", (bucket_id,)).fetchall()
            conn.close()
            return [{"target_id": r[0], "edge_type": r[1], "weight": r[2], "created_at": r[3]} for r in rows]
        except Exception as e:
            import logging; logging.getLogger(__name__).warning(f"list_edges failed: {e}")
            return []
