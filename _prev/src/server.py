...
try:
    ...
    from night_fall.config import NightFallConfig as _NightFallConfig
    _NIGHTFALL_AVAILABLE = True
except ImportError:
    _NIGHTFALL_AVAILABLE = False
...
if _NIGHTFALL_AVAILABLE:
    try:
        import os as _os
        _os.environ['OMBRE_HOME'] = '/opt/ombre-brain/src'
        _os.environ.setdefault('NIGHT_FALL_DATA_DIR', '/opt/ombre-brain/buckets/night_fall')
        from night_fall.config import load_config as _nf_load_config
        from night_fall.tool import night_fall_tool as _nf_tool, get_surfaceable_dream as _nf_get_surfaceable
        _nf_cfg = _nf_load_config(require_ombre=False)

        # Wrapper so OmbreAdapter can access server globals
        class _NFServer:
            config = config
            dehydrator = dehydrator
            embedding_engine = embedding_engine
            bucket_mgr = bucket_mgr
        _nf_server = _NFServer()
        ...
        async def night_fall(
            ...
            """Night Fall latent dream lifecycle. action=generate|surface|status|cleanup"""
            return await _nf_tool(
                _nf_server, _nf_cfg,
                action=action, query=query,
                current_valence=current_valence, current_arousal=current_arousal,
                current_motifs=current_motifs, is_session_start=is_session_start,
                debug=debug,
            )

        async def _nf_auto_surface():
            return await _nf_get_surfaceable(_nf_server, _nf_cfg)
        _tools_runtime.init(night_fall_auto_surface=_nf_auto_surface)
        logger.info('Night Fall extension loaded OK')
    except Exception as _nf_err:
        import logging as _nf_logging
        _nf_logging.getLogger('ombre_brain').warning(f'Night Fall init failed: {_nf_err}')
...
if __name__ == "__main__":
    ...
    if transport in ("sse", "streamable-http"):
        ...
        if transport == "streamable-http":
            # iter 2.2：先把 mcp_extra 工具回灌进 mcp——让 /mcp 单端点暴露 12 个工具。
            try:
                mcp._tool_manager._tools.update(mcp_extra._tool_manager._tools)
                logger.info(f"streamable-http 单连接器：已回灌 {len(mcp_extra._tool_manager._tools)} 个副集工具进主实例")
            except AttributeError as _merge_exc:
                logger.warning(f"工具回灌失败，仅暴露主集工具：{_merge_exc}")
