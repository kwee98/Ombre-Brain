...
async def dispatch(
    ...
    if not query or not query.strip():
        ...
        # Night Fall: 有 valence/arousal 时尝试浮现潜梦
        is_contextual = (valence != -1 or arousal != -1)
        if is_contextual and rt.night_fall_auto_surface:
            try:
                dream_block = await rt.night_fall_auto_surface()
                if dream_block:
                    result = result + nn + dream_block
            except Exception:
                pass
        return result
